...
public class Main {
	...
	public static void init(Scanner theScanner) throws FileNotFoundException {		
		...
		System.out.print("Please pick the color to play! (b or w) ");
	 ...
	 */
	public static void computerMove(boolean isAlpha) {		
		AI aI = new AI();
		int[] pickMoveAI = null;
		if (ALPHA_BETA) { //choose alpha beta or min max algorithm
			pickMoveAI = aI.alphaBeta(DEPTH, myGameBoard, isAlpha, Integer.MIN_VALUE, Integer.MAX_VALUE);
		} else {
			pickMoveAI = aI.minMax(DEPTH, myGameBoard, isAlpha);			
		}
		...
		System.out.println("Nodes Expanded: " + aI.getNodes() + ", Depth Limit: " + DEPTH);
	}
	...
	public static void playGame(Scanner theScanner, boolean humanFirstPlay) {
		...
		if (stats == 't') System.out.println("Game Tie!");
		else System.out.println("The "+ stats + " Token player won!");
	}
}